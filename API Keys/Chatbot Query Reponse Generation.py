...
def handle_query_with_context_ws(df: pd.DataFrame,
                                 query: str,
                                 state: dict,
                                 top_k: int = TOP_K_DEFAULT):
    """
    WebSocket version of handler with conversation context.
    - df: current CSV dataframe
    - query: current user question
    - state: dict with keys: last_query, last_result, last_top_product_title
    Returns: (result_dict, updated_state)
    """
    fallback_msg = (
        "Sorry, I couldn't answer that. "
        "I can assist you with product, website, business or item-related queries."
    ...
    # If irrelevant → clear last product context
    if is_irrelevant(query):
        result = {
            "query": query,
            "top_results": [],
    ...
    # Decide if we should treat this as a follow-up to last product
    use_followup = looks_like_followup(query, state)

    # Build TF-IDF index fresh per request (like handle_query_with_uploaded_csv)
    text_field = choose_text_field(df)
    vectorizer, tfidf_matrix = build_tfidf_index(df, text_field)
    ...
    # ---------- Column-aware answering (same as before) ----------
    intent = detect_question_intent(query)
    top = retrieved.iloc[0]
    ...
    # Update state: remember this product for future follow-ups
    state["last_query"] = query
    state["last_result"] = retrieved
